...
정보보안 분야에 ML을 적용하는 것은 여전히 어렵고 도전적인 영역입니다.

현실 세계의 네트워크 트래픽에서 **비정상(이상) 행위의 비율은 절대적으로 낮습니다.** 정상 트래픽이 99% 이상을 차지하는 극심한 클래스 불균형 환경에서, Isolation Forest나 LOF 같은 비지도 Outlier 탐지 방식을 사용하더라도 모수 자체가 수만~수십만 건에 달하기 때문에, 아주 작은 오탐률(False Positive Rate)조차 **수백~수천 건의 노이즈 알럿**으로 이어집니다. 🔔 알럿 피로(Alert Fatigue)는 결국 진짜 위협을 놓치게 만드는 역설을 낳습니다.

🚧 그렇다고 전통적인 **Rule 기반 탐지**만으로는 한계가 명확합니다. 공격자는 고정된 시그니처와 임계값을 학습하여 우회하고, 저강도 지속 위협(Low-and-Slow Attack)이나 점진적 데이터 유출처럼 개별 패킷/세션 단위로는 정상처럼 보이는 위협은 규칙으로 포착하기 어렵습니다.

🔗 결국 현실적인 해답은 **ML과 Rule 기반 탐지의 결합(Hybrid Approach)** 입니다. ML은 알려지지 않은 패턴을 통계적으로 포착하고, Rule은 명확한 보안 시그널(C2 비콘, 포트 불일치, 고위험 국가 접속 등)을 확실하게 잡아냅니다. 두 방식이 서로의 약점을 보완할 때 탐지 커버리지와 정확도가 모두 높아집니다.
...
네트워크 트래픽 데이터는 **극단적인 스큐(skew)** 가 특징입니다. 현실의 트래픽 분포는 대략 아래와 같습니다:

```
대부분의 IP (99%):   1,000 ~ 50,000 bytes    ← 좁은 범위에 밀집
일부 특이 IP  (1%):  1,000,000+ bytes         ← 극단적 이상치(outlier)
```

이 상태에서 **`StandardScaler`** (평균/표준편차 기반)를 사용하면, 극단값이 평균과 표준편차를 크게 왜곡합니다. 결과적으로 정상 트래픽 99%가 "평균 이하"로 납작하게 압축되어, ML 모델이 정상과 이상을 제대로 구분하지 못하게 됩니다.

```python
# StandardScaler: (x - mean) / std    → 이상치가 mean/std를 왜곡 → 정상범위가 압축됨
# RobustScaler:   (x - median) / IQR  → 이상치에 흔들리지 않음 → 정상범위를 안정적으로 표현
```

**`RobustScaler`** 는 **중앙값(Median)** 과 **IQR(사분위 범위, Q3-Q1)** 을 기준으로 정규화합니다. 극단값이 아무리 크더라도 중앙값과 IQR은 거의 변하지 않기 때문에, 대부분의 정상 트래픽을 일관된 척도로 표현하고 모델이 올바른 기준으로 이상을 탐지할 수 있습니다.
