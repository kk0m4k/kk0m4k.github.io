...
LLM을 사용할 때 가장 큰 고민 중 하나는 **다양한 LLM 모델을 어떻게 통합 관리할 것인가**입니다. OpenAI, Anthropic Claude, AWS Bedrock 등 여러 Provider를 사용하면서 비용 추적, 접근 제어, 로깅, 및 보안정책을 일관되게 관리하는 것은 쉽지 않습니다. 특히 전자금융감독규정의 망분리 및 클라우드컴퓨팅서비스 이용 절차 등을 준수해야 하는 업권에서는 컴플라이언스 측면에서 고려해야 하는 것들이 있습니다.

- 🏢 **내부망(5호망)**: 생성형 AI 사용 시 **혁신금융서비스(혁금) 지정 신청**이 필수
- 🔬 **연구개발망**: 혁신금융서비스 지정 없이 사용 가능하나, **자체 보안 요구사항을 엄격**하게 적용
- 📝 **감독규정 제 14조의 클라우드 이용 절차 준수(제 14의 2)** : 고유식별정보 또는 개인신용정보를 처리하는 경우에는 정보처리시스템을 `국내`에 설치하여야 한다. 이에 따라서 사용하는 모델도 서울 리전 필수. 가명처리된 개인(신용)정보도 해당이되기때문에 국내 리전으로 국한.

금융보안원에서 제시한 **「생성형 AI 연계 이용 보안대책」**에서는 프롬프트 인젝션 방어, 민감정보 유출 방지, 입출력 로깅 등의 기술적 보안 조치를 요구합니다. LiteLLM의 **Guardrail**(Prompt Injection 탐지, PII 마스킹)과 **Callback 기반 로깅**을 활용하면 이러한 보안 요구사항을 상당 부분 기술적으로 충족할 수 있습니다.

이 글에서는 **LiteLLM**을 Gateway(Proxy)로 활용하여 AWS Bedrock 모델들을 통합 관리하고, AWS Data Firehose를 통한 감사 로깅을 구현하는 방법에 대해서 기술하였습니다.
...
이를 통해 특정 리전 장애 시에도 서비스 연속성을 보장합니다. 다만, 전자금융감독규정을 준수해야 하는 경우은 반드시 서울 리전에서만 제공되는 AWS Bedrock 모델을 사용해야 합니다.
